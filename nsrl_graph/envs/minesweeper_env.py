...
class MinesweeperEnv(object):
    ...
    def get_neighbors(self, coord):  # can be ['0', 'B', 0]
        x,y = coord[0], coord[1]
    ...
    def get_neighbors_stateim(self, coord):  
        x,y = coord[0], coord[1]
        ...
        for col in range(y-1, y+2):
            for row in range(x-1, x+2):
                if ((x != row or y != col) and
                    (0 <= col < self.ncols) and
                    (0 <= row < self.nrows)):
                    neighbors.append(self.state_im[row][col])

        return np.array(neighbors)
        
        
    def obtain_boundary(self):
        boundaries = []
        for col in range(self.ncols):
            for row in range(self.nrows):
                neighbors = self.get_neighbors_stateim((row, col))
                print(neighbors)
                if self.state_im[row][col] == -0.125 and any(t>0 for t in neighbors): 
    ...
    def step(self, action_index):
        ...
        boundary_index = self.obtain_boundary()
