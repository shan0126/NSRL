...
########################################
    ...
    @torch.no_grad()
    def get_mpe_condi(self, batch_size, ava, clear_data=True):
        """Compute the MPE instation given weights associated with literals.
        ...
        for node in self.as_positive_list(clear_data=clear_data):
            iter_id += 1
            node.isposns = torch.tensor([0], device=DEVICE, dtype=tint).unsqueeze(0).unsqueeze(-1).expand(batch_size, 1, self.num_reps)
            # node.isnegns = torch.tensor([0], device=DEVICE, dtype=tint).unsqueeze(0).unsqueeze(-1).expand(batch_size, 1, self.num_reps)
            node.isnegns = torch.tensor([-999], device=DEVICE, dtype=tint).unsqueeze(0).unsqueeze(-1).expand(batch_size, 1, self.num_reps)
            node.ns_pos = torch.tensor([0], device=DEVICE, dtype=tint).unsqueeze(0).unsqueeze(-1).expand(batch_size, 1, self.num_reps)
            ...
            if node.is_false():
                ...
                print("warning: haven't done in sdd is_true get_mpe_condi")
                exit(0)
                ...
                str_log = "iter: {}; var: {}; literal: {}".format(iter_id, node.vtree.var, node.literal)
                log.append(str_log)
                ava_this_var = ava[:, node.vtree.var - 1]
                ava_this_var = ava_this_var.unsqueeze(-1).unsqueeze(-1)
                if node.literal > 0:
                    node.ns_pos = torch.where(ava_this_var == 0, node.isnegns, node.isposns)
                    # if int(node.literal) == 14 and (ava_this_var == 0).any():
                    #     print("======================================")
                    #     print(node.literal)
                    #     print(ava)
                    #     print(node.ns_pos)
                    #     print("======================================")
                    # if int(node.literal) == 4 and (ava_this_var == 0).any() and (ava_this_var == 1).any():
                    #     print(node.literal)
                    #     print(ava)
                    #     print(node.ns_pos)
                    #     exit(0)
                ...
                print("warning: haven't done in sdd get_mpe_condi")
                exit(0)
                ...
                if len(node.positive_elements) == 1:
                    ...
                    # str_log += "p: {}, s: {}; ".format(p.data, s.data)
                    node.ns_pos = p.ns_pos + s.ns_pos
                    #max_branch = torch.zeros((1, batch_size, self.num_reps), device=DEVICE, dtype=torch.int16)
                else:
                    new_theta = []
                    ns_pos_all = torch.tensor([0], device=DEVICE, dtype=tint).unsqueeze(0).unsqueeze(-1).expand(batch_size, 1, self.num_reps)
                    for i, child_theta in enumerate(node.theta):
                        p,s = node.elements[i]
                        ns_pos_all = ns_pos_all + p.ns_pos + s.ns_pos
                        new_theta.append(child_theta + p.ns_pos.squeeze(-1) + s.ns_pos.squeeze(-1))
                        # str_log += "p: {}, s: {}; ".format(p.data, s.data)
                        # data += child_theta * p.data * s.data
                    new_theta = torch.stack(new_theta, dim = 0)
                    node.ns_pos = ns_pos_all
                    max_branch = new_theta.argmax(dim=0, keepdim=True)
        ...
        # self.checkconsis(batch_size, data.gather(1, indices), ava, log)
        return data.gather(1, indices)


    def checkconsis(self, batch_size, indices, ava, log):
        argmax = self.mixing.argmax(dim=-1)
        indices = indices[torch.arange(batch_size), :, argmax]
        indices = (indices > 0).long()
        indices = indices.reshape(-1, 2, 7)
        indices = indices.max(dim=2)[1]
        ava = ava.reshape(-1, 2, 7)
        
        indices_expanded = indices.unsqueeze(-1).expand(-1, -1, ava.size(-1))
        selected_values = torch.gather(ava, dim=-1, index=indices_expanded)
        
        result = (selected_values == 0).any(dim=-1)  
        if result.any():
            print("indices:\n", indices)
            print("ava:\n", ava)
            ...
            for l in log:
                print(l)
            raise ValueError("Found True in result. Program interrupted.")
