...
def main(args, env_name, number, seed):
    ...
    args.state_img_height = size * 28
    args.state_img_width = size * 28
    args.nrows = size
    args.ncols = size
    ...
    gate = DenseGatingFunction(cmpe.beta, args, gate_layers=[args.state_dim]+[args.num_units_gate]*args.num_layers_gate, num_reps=args.num_reps_gate).to(device)
    ...
    with open(csv_file, mode='w', newline='') as file:
        ...
        writer.writerow(["Total Steps", "Evaluate Reward", "Epsilon", "Violation", "Individual_acc"])
    ...
    while total_steps < args.max_train_steps:
        ...
        if episode_idx % args.evaluate_freq_epi == 0:
            ...
            with open(csv_file, mode='a', newline='') as file:
                writer = csv.writer(file)
                writer.writerow([total_steps, evaluate_reward, epsilon, episode_violation/episode_idx if episode_idx>0 else 0, 0])
                
            if episode_idx > 0:
                # evaluate SPL
                gate.eval()
                index_id = min(current_size, 1000)
                X = torch.tensor(train_data[:index_id]).float().to(device)
                y = torch.tensor(train_label[:index_id]).float().to(device)
                
                X = agent._flat_batch_to_img_tensor(X)
        ...
        while not done:
            ...
            if performa is not None:
                ...
                if key not in seen_samples:
                    ...
                    current_ptr = (current_ptr + 1) % 1000
